...
- **`turbo.json` `build.env` must list every Next.js-injected env var** (`API_URL`, `NEXT_PUBLIC_BACKEND_URL`, etc.). Turborepo only forwards variables declared there — without it, `process.env.*` gets baked as `undefined` in the Next.js build. Express does not need this because it reads env vars at runtime via `dotenv`. When adding a new env var to the frontend, always add it to `turbo.json` too.
